...
<Bank version="1">
    <Section name="unit"> // in section unit, all the runling data is saved
        ...
        <Key name="info">  // Slots information
            <Value string="/...*qunHr 7h"/>  // "..." inserted to remove compiler warning
        </Key>
        <Key name="05">
...
std::vector<int> bankgen::decode_account_data(const std::string& input_string) {
    size_t kNumAccountAttributes = rlr4::account_data_maxvals.size();
    std::vector<int> account_data(kNumAccountAttributes);
    std::string decrypted_string = starcode.decrypt(input_string, rlr4::key);
    std::string decompressed_string = starcode.decompress(decrypted_string);
    starcode.set_code(decompressed_string);
    for (size_t i = kNumAccountAttributes - 1; i < kNumAccountAttributes; --i) {
        int max_val = rlr4::account_data_maxvals[i];
        account_data[i] = starcode.get_int(max_val);
    }
    return account_data;
}
...
std::vector<int> bankgen::decode_runling_data(const std::string& input_string) {
    size_t kNumAttributes = rlr4::runling_data_maxvals.size();
    std::vector<int> runling_data(kNumAttributes);
    std::string decrypted_string = starcode.decrypt(input_string, rlr4::key);
    std::string decompressed_string = starcode.decompress(decrypted_string);
    starcode.set_code(decompressed_string);
    for (size_t i = kNumAttributes - 1; i < kNumAttributes; --i) {
        int max_val = rlr4::runling_data_maxvals[i];
        runling_data[i] = starcode.get_int(max_val);
    }
    return runling_data;
}
...
void bankgen::decode_checksums(const std::string& input_string, int* account_checksum,
                               int* character_checksum) {
    std::string decrypted_string = starcode.decrypt(input_string, rlr4::key);
    std::string decompressed_string = starcode.decompress(decrypted_string);
    starcode.set_code(decompressed_string);
    *character_checksum = starcode.get_int(98000000);
    *account_checksum = starcode.get_int(99000000);
}
