...
int bankgen::compute_character_checksum(std::vector<std::vector<int>> &all_character_data, int handle = 0) {
...
std::string bankgen::compute_signature(int player_handle, std::vector<std::vector<std::vector<std::string>>> bank_tags) {
    std::string to_hash("");
    // We start by concatenating the author handle, player handle, and bank name
    std::string region_prefix = "1-S2-1-";
    to_hash += region_prefix + std::to_string(rlr4::author_handle);
    to_hash += region_prefix + std::to_string(player_handle);
...
std::string bankgen::generate_bank(std::vector<int> account_data, std::vector<std::vector<int>> runling_data,
    ...
    // save account data
    s += "\t\t<Key name=\"info\">\n";
    string account_code = bankgen::encode_account_data(account_data);
    ...
    // ** Signature **
    std::string signature = bankgen::compute_signature(handle, bank_tags);
...
void bankgen::write_to_file(std::string filename, std::string s) {
    ...
    ofs << s;
    ofs.close();
    std::cout << "Bank generated and saved to " << filename << endl;
}
...
string parse_line_string(ifstream &fin) {
    ...
    while (getline(fin, line)) {
        if (!bankgen::string_startswith(line, comment_delim)) {
            // add an extra check here: we only read lines with ":" in them
            size_t colon_pos = line.find(':');
            if (colon_pos != string::npos) {
...
int main() {
    ...
    for (int i = 0; i < 8; i ++) {
        ...
        int used_points = bankgen::calc_points_used(results[2], results[3], results[4], results[5], results[6]);
    ...
    for (unsigned int i = 0; i < runling_data.size(); i++) {
        cout << "Slot " << slot_numbers[i] << ": " << bankgen::vtos(runling_data[i]) << endl;
    }
    cout << endl;
    ...
    fout << bankgen::generate_bank(account_data, runling_data, slot_numbers, handle);
    fout.close();
